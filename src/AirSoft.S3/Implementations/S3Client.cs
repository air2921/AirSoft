...
public class S3Client(IAmazonS3 s3Client) : IS3Client
{
    ...
    public async Task<S3ObjectUrlDetails> SignUrlAsync(string bucket, string key, HttpVerb verb, DateTime expires)
    {
        try
        {
            var response = new S3ObjectUrlDetails
            {
                ...
                Key = key,
                Verb = verb,
                Url = await s3Client.GetPreSignedURLAsync(new GetPreSignedUrlRequest
                {
                    BucketName = bucket,
                    Key = key,
                    Expires = expires,
        ...
        {
            throw new S3ClientException("An error occurred while attempting to create a signed reference to an object", ex);
        }
    }
    ...
    public async Task<S3ObjectContentDetails> DownloadAsync(string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new S3ClientException("An error occurred while trying to unload an object from storage", ex);
        }
    }
    ...
    public async Task UploadAsync(Stream stream, string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new S3ClientException("An error occurred while trying to load the object into storage", ex);
        }
    }
    ...
    public async Task RemoveAsync(string bucket, string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new S3ClientException("An error occurred while attempting to delete an object from storage", ex);
        }
    }
}
