...
namespace AirSoft.EntityFrameworkCore.DbContexts
{
    ...
    public abstract class AuditableDbContext : DbContext
    {
        ...
        private void OnBeforeSaving()
        {
            ...
            foreach (var entry in entries)
            {
                if (entry.Entity is EntityBase entity)
                {
                    switch (entry.State)
                    {
                        case EntityState.Deleted when entity is SoftableEntityBase softableEntity:
                            OnSoftRemoving(entry, softableEntity);
                            break;

                        case EntityState.Deleted when entity is ForceableEntityBase forceableEntity:
                            OnHardRemoving(entry, forceableEntity);
                            break;
        ...
        /// <summary>
        /// Handles physical deletion of entities that inherit from <see cref="ForceableEntityBase"/>.
        /// </summary>
        /// <param name="entry">The change tracker entry for the entity.</param>
        ...
        /// </remarks>
        protected virtual void OnHardRemoving(EntityEntry entry, ForceableEntityBase entity)
        {
            return;
        }

        /// <summary>
        /// Handles soft deletion of entities that inherit from <see cref="SoftableEntityBase"/>.
        /// </summary>
        /// <param name="entry">The change tracker entry for the entity.</param>
        ...
        /// </remarks>
        protected virtual void OnSoftRemoving(EntityEntry entry, SoftableEntityBase entity)
        {
