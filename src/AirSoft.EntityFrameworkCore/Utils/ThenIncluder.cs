...
namespace AirSoft.EntityFrameworkCore.Utils
{
    ...
    public class ThenIncluder<TEntity, TPreviousProperty> : Includer<TEntity> where TEntity : EntityBase
    {
        /// <summary>
        ...
        /// </summary>
        private readonly Includer<TEntity> _rootIncluder;
        ...
        /// </summary>
        internal ThenIncluder(Includer<TEntity> rootIncluder, string currentPath)
        {
        ...
        public ThenIncluder<TEntity, TProperty> WithThenInclude<TProperty>(Expression<Func<TPreviousProperty, TProperty>> expression) where TProperty : EntityBase
        {
        ...
        public ThenIncluder<TEntity, TProperty> WithThenInclude<TProperty>(Expression<Func<TPreviousProperty, IEnumerable<TProperty>>> expression) where TProperty : EntityBase
        {
            _ = expression ?? throw new InvalidArgumentException($"Using {nameof(WithThenInclude)} without expression is not allowed");
            string nestedPath = GetNestedPathFromExpression(expression);
            string fullPath = $"{_currentPath}.{nestedPath}";
        ...
        public new ThenIncluder<TEntity, TProperty> WithInclude<TProperty>(Expression<Func<TEntity, TProperty>> expression) where TProperty : EntityBase
        ...
        public new ThenIncluder<TEntity, TProperty> WithInclude<TProperty>(Expression<Func<TEntity, IEnumerable<TProperty>>> expression) where TProperty : EntityBase
            => _rootIncluder.WithInclude(expression);
    }
}
