...
namespace AirSoft.EntityFrameworkCore.Utils
{
    ...
    /// </summary>
    /// <typeparam name="TEntity">The root entity type to include relations for, must inherit from EntityBase.</typeparam>
    public class Includer<TEntity> where TEntity : EntityBase
    {
        /// <summary>
        ...
        public ThenIncluder<TEntity, TProperty> WithInclude<TProperty>(Expression<Func<TEntity, TProperty>> expression) where TProperty : EntityBase
        {
        ...
        public ThenIncluder<TEntity, TProperty> WithInclude<TProperty>(Expression<Func<TEntity, IEnumerable<TProperty>>> expression) where TProperty : EntityBase
        {
            _ = expression ?? throw new InvalidArgumentException($"Using {nameof(WithInclude)} without expression is not allowed");
            string path = GetPathFromExpression(expression);
            Paths.Add(path);
