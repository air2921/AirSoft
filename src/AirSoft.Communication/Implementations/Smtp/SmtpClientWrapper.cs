    ...
    public class SmtpClientWrapper : IDisposable
    {
        ...
        public SmtpClientWrapper(SmtpConfigureOptions configureOptions)
        {
            _transport = new Lazy<IMailTransport>(() =>
            {
                ...
                {
                    throw new SmtpClientException("Failed to authenticate or connect to the SMTP server", ex);
                }
        ...
        public async Task SendAsync(MimeMessage message, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new SmtpClientException("Error during email sending due to authentication or network issues", ex);
            }
