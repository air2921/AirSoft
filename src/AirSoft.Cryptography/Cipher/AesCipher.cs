...
namespace AirSoft.Cryptography.Cipher
{
    /// <summary>
    ...
    public class AesCipher : ICipher
    {
        ...
        public async Task EncryptAsync(Stream source, Stream target, byte[] key, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new CryptographyException("An error occurred while attempting to encrypt data", ex);
            }
        }
        ...
        public async Task DecryptAsync(Stream source, Stream target, byte[] key, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    int read = await source.ReadAsync(iv.AsMemory(bytesRead), cancellationToken);
                    if (read == 0)
                        throw new CryptographyException("Unexpected end of stream while reading IV");
                    bytesRead += read;
                }
                aes.IV = iv;
            ...
            {
                throw new CryptographyException("An error occurred while attempting to decrypt data", ex);
            }
        }
        ...
        public void Decrypt(Stream source, Stream target, byte[] key)
        {
            ...
            try
            {
                ...
                while (bytesRead < iv.Length)
                {
                    int read = source.Read(iv.AsSpan(bytesRead));
                    if (read == 0)
                        throw new CryptographyException("Unexpected end of stream while reading IV");
                    bytesRead += read;
                }
